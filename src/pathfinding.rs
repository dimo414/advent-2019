...
mod tests {
    ...
    fn all_paths() {
        ...
        let bfs_routes_lens: HashMap<_,_> = bfs_routes.iter().map(|(&k, v)| (k, v.len())).collect();
        let expcted_routes: HashMap<_,_> = vec!(
            (point(1, 1), 4), (point(2, 1), 5), (point(1, 2), 3), (point(3, 2), 3),
            (point(4, 2), 4), (point(1, 3), 2), (point(2, 3), 1), (point(3, 3), 2)
        ).iter().cloned().collect();
        assert_eq!(bfs_routes_lens, expcted_routes);

        let bfs_route = graph.bfs(&start, &farthest).unwrap();
